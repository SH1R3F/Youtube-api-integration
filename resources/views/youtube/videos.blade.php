        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-10">
                ...
                <div class="flex justify-between mt-7">
                    @if (isset($videos->prevPageToken))
                        <a href="{{ route('youtube.videos', ['pageToken' => $videos->prevPageToken, 'channel_id' => $channelId]) }}"
                            class="bg-blue-500 px-3 py-1 rounded-md text-white">Prev</a>
                    @else
                        <a></a>
                    @endif
                    @if (isset($videos->nextPageToken))
                        <a href="{{ route('youtube.videos', ['pageToken' => $videos->nextPageToken, 'channel_id' => $channelId]) }}"
